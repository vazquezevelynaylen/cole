...
export default function Header({ isSolid = false }: HeaderProps) {
  useEffect(() => {
    ...
    const setOpen = (open: boolean) => {
      ...
      if(open){
        const first = nav.querySelector('a,button') as HTMLElement
        first && first.focus()
      }else{
        toggle.focus()
      }
  ...
  useEffect(() => {
    ...
    items.forEach(li => {
      const btn = li.querySelector('button[aria-haspopup="true"]') as HTMLButtonElement
    ...
    const onEscAll = (e: KeyboardEvent) => {
      if(e.key === 'Escape'){
